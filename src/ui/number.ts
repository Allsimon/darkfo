module Ui {
    export class NumberContainer extends WindowFrame {
        ...
        constructor(public bbox: CSSBoundingBox,
                    private maxDigit = 4) {
        ...
        static autoBoundingBox(bbox: CSSBoundingBox, maxDigit: number) {
            if (!bbox.h && !bbox.w) {
                bbox.w = (NumberWidget.CHAR_WIDTH + NumberWidget.CHAR_WIDTH_PADDING * 2) * maxDigit;
        ...
        private setAmountAnimated(currentAmount: number, targetAmount: number, onFinish: () => void) {
            if (currentAmount === targetAmount) {
                onFinish();
                return;
            }
            // Don't animate for the first animation
            const amount = currentAmount === undefined ? targetAmount : (currentAmount < targetAmount ? 1 : -1) + currentAmount;
            ...
            // -999 === 10^3 - 1
            // 9999 === 10^4 - 1
            const maxAmount = Math.pow(10, (this.maxDigit + (isNegative ? -1 : -1))) - 1;
            ...
            for (let i = 0; i < this.maxDigit; i++) {
                ...
                this.children[i]
                    .css({backgroundPositionX: this.computeBackgroundPosition(i, safeAmount, gravity) + PX});
            }

            // todo: find the correct value, is it even linear ?
            const ANIMATION_DURATION = 32;
            setTimeout(() => this.setAmountAnimated(amount, targetAmount, onFinish), ANIMATION_DURATION);
        }

        leftPadNegative(value: number, isNegative: boolean) {
            const stringMaxSize = this.maxDigit + (isNegative ? -1 : 0);
            const leftPadded = new Array(this.maxDigit).concat([value]).join('0').slice(-stringMaxSize);
            return (isNegative ? '-' : '') + leftPadded;
        }

        computeBackgroundPosition(index: number, amount: string, gravity = Gravity.NA): number {
            const digit = amount[index] === '-' ? NumberWidget.CHAR_NEG : +amount[index];
