namespace Ui {
  export class CharacterScreen extends WindowFrame {
    ...
    constructor() {
      ...
      this.add(new SmallButton(455, 454).onClick(() => {}))
        ...
        .add(
          new Label(33, 280, `Level: ${critterGetStat(player, "Level")}`).css({
            fontSize: "0.75em",
            color: "#00FF00"
          })
        )
        .add(
          new Label(
            33,
            292,
            `Exp: ${critterGetStat(player, "Experience")}`
        ...
        .add(
          new Label(
            194,
            45,
            `Hit Points ${critterGetStat(player, "HP")}/${critterGetStat(
              player,
              "Max HP"
            )}`
          ).css({ fontSize: "0.75em", color: "#00FF00" })
    ...
    refresh() {
      ...
      for (const stat of stats) {
        const widget = new Label(20, 39 + n, "").css({
          background: "black",
          padding: "5px"
        });
        widget.onClick(() => {
      ...
      const skillPointCounter = new Label(522, 230, "").css({
        background: "black",
        padding: "5px"
      });
      ...
      const redrawStatsSkills = () => {
        ...
        for (const skill of skills)
          this.skillList.addItem({
            text: `${skill} ${newSkillSet.get(skill, newStatSet)}%`,
            id: skill
          });
      ...
      if (isLevelUp) {
        const modifySkill = (inc: boolean) => {
          ...
          console.log(
            "skill: %s currently: %d",
            skill,
            newSkillSet.get(skill, newStatSet)
          );
